...
library(limma)
library(gridExtra)
setwd("C:\\microRNA\\Chalk talk")
```

Data: miRNA data & design matrix
```{r read miRNA data}
# Load in 3 data files
villi <- as.matrix(read.delim("C:\\microRNA\\01 - Input data\\01B_miRNA_placenta_newer_info.txt", 
															row.names = 1))
tissue <- as.matrix(read.delim("C:\\microRNA\\01 - Input data\\02B_allTissues_miRNA_Fetal_edited.txt", 
															 row.names = 1))
lung <- as.matrix(read.delim("C:\\microRNA\\01 - Input data\\02C_Last batch 10 lung fetal miRBase20_edited.txt", 
									 row.names = 1))
...
desMat <- read.delim("C:\\microRNA\\01 - Input data\\02A_desMat_allTissues.txt", row.names = 1)
...
```{r}
anno <- read.delim("C:\\microRNA\\02 - annotation\\miRBase_v20_hsa.gff3_clean.txt")
str(anno)
...
heatmap.3(log10(miRNA+1),hclustfun=myclust, distfun=mydist,na.rm = TRUE, scale="none", 
          ...
          KeyValueName="log10(exp+1)",keysize=1.2,
          cexCol=0.8)
...
```{r PCA}
sum(is.na(miRNA)) # 0 
source("C:/450k analyses/MTHFR project/heat.scree_allPCs.R")
 ...
 **08.J Volcano plots**
```{r 08.J VILLI volcano plots - all probes}
processingTop$FDR<- as.factor(processingTop$adj.P.Val < 0.05)
head(processingTop)

#pdf(file="C:\\Thesis\\Chapter 5\\Figures\\volcano_677.pdf",width=3.5, height=3.5)
ggplot(processingTop.all.80, aes(x=logFC, y=-log10(adj.P.Val)),) +
	geom_point(shape=19, alpha=0.5, size=3, color = "slategray") + 
	theme_bw() +
	geom_point(data = subset(processingTop.all.80, adj.P.Val <= 0.05),
						 aes(x=logFC, y=-log10(adj.P.Val)),shape=19, alpha=0.5, size=3, color = "red") + 
	geom_point(data = processingTop.all.80[rownames(processingTop.all.80) %in% 
																									rownames(trimester.all.FDR05),],
						 aes(x=logFC, y=-log10(adj.P.Val)),shape=19, alpha=0.5, size=3, color = "blue") +
	geom_hline(yintercept=(-log10(0.05)), color="slategray", size=.3) +
	coord_cartesian(xlim = c(-0.02, 0.02)) +
  ylab("-log10(adjusted P.Value)") + 
  xlab("log2 fold change")+
  ggtitle("Change in microRNA expression with processing time" )+
  annotate("text", -0.02,(-log10(0.045)) ,label = "FDR <0.05", size=3)
 # dev.off()
```
...
# What is expressed in all tissues & all samples?
head(prop.ByTissue[,1])
head(prop.ByTissue[,1] == 1)
head(which(prop.ByTissue[,1] == 1))


head(prop.ByTissue[,2], n=20L)
head(prop.ByTissue[,2] == 0, n=20L)
head(which(prop.ByTissue[,2] == 0), n=20L)

str(names(which(prop.ByTissue[,1] == 1)))
dimnames(prop.ByTissue)[[2]]
#[1] "chorionic villi_1" "brain_2"           "chorionic villi_2" "kidney_2"          "liver_2"           "lung_2"           
#[7] "muscle_2"          "spinal cord_2"     "chorionic villi_3 

#141 expressed in all tissues, all samples
length(common.all <- Reduce(intersect, list(names(which(prop.ByTissue[,1] == 1)), names(which(prop.ByTissue[,2] == 1)),
																	 names(which(prop.ByTissue[,3] == 1)), names(which(prop.ByTissue[,4] == 1)),
																	 names(which(prop.ByTissue[,5] == 1)),names(which(prop.ByTissue[,6] == 1)),
																	 names(which(prop.ByTissue[,7] == 1)),names(which(prop.ByTissue[,8] == 1)),
																	 names(which(prop.ByTissue[,9] == 1))
																	 )))

#0 expressed in somatic tissue, all samples, no chorionic villi
length(common.somatic <- Reduce(intersect, list(names(which(prop.ByTissue[,1] == 0)), names(which(prop.ByTissue[,2] == 1)),
																	 names(which(prop.ByTissue[,3] == 0)), names(which(prop.ByTissue[,4] == 1)),
																	 names(which(prop.ByTissue[,5] == 1)),names(which(prop.ByTissue[,6] == 1)),
																	 names(which(prop.ByTissue[,7] == 1)),names(which(prop.ByTissue[,8] == 1)),
																	 names(which(prop.ByTissue[,9] == 0))
																	 )))

#0 expressed only in 1st trimester villi
length(common.somatic <- Reduce(intersect, list(names(which(prop.ByTissue[,1] > 0.5)), names(which(prop.ByTissue[,2] == 0)),
																	 names(which(prop.ByTissue[,3] == 0)), names(which(prop.ByTissue[,4] == 0)),
																	 names(which(prop.ByTissue[,5] == 0)),names(which(prop.ByTissue[,6] == 0)),
																	 names(which(prop.ByTissue[,7] == 0)),names(which(prop.ByTissue[,8] == 0)),
																	 names(which(prop.ByTissue[,9] == 0))
																	 )))			
 

length(common.somatic <- Reduce(intersect, list(names(which(prop.ByTissue[,1] == 0)), names(which(prop.ByTissue[,2] == 0)),
																	 names(which(prop.ByTissue[,3] > 0.5)), names(which(prop.ByTissue[,4] == 0)),
																	 names(which(prop.ByTissue[,5] == 0)),names(which(prop.ByTissue[,6] == 0)),
																	 names(which(prop.ByTissue[,7] == 0)),names(which(prop.ByTissue[,8] == 0)),
																	 names(which(prop.ByTissue[,9] == 0))
																	 )))	
...
length(common.somatic <- Reduce(intersect, list(names(which(prop.ByTissue[,1] == 0)), names(which(prop.ByTissue[,2] == 0)),
																	 ...
																	 names(which(prop.ByTissue[,5] == 0)),names(which(prop.ByTissue[,6] == 0)),
																	 names(which(prop.ByTissue[,7] == 0)),names(which(prop.ByTissue[,8] == 0)),
																	 names(which(prop.ByTissue[,9] > 0.5))
																	 )))
```
