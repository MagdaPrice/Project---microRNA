...
```{r read data}
piRNA <- read.delim("C:\\microRNA\\01 - Input data\\Placenta-piRNA-expression_edited.txt", row.names = 1)
head(piRNA)
colnames(piRNA)
piRNA <- as.matrix(piRNA)

desMat <- read.delim("C:\\microRNA\\01 - Input data\\desMat_controls.txt", row.names = 1)
head(desMat)
rownames(desMat)
...
```{r}
corMat <- desMat[,c(1,8,9,10)]
corMat$Tech1 <- as.numeric(corMat$Tech1)
...
row.tech1=gsub("MX1303", "#D9D9D9",gsub("MX1307", "#F7F7F7", gsub("MX1310", "#BDBDBD", gsub("MX1355", "#737373",
																				gsub("MX1356", "#525252", gsub("MX1356", "#969696", gsub("MX1357", "#252525", desMat$Tech1)))))))

row.tech2=gsub("C5JC1ACXX", "#F7F7F7", gsub("C5JC4ACXX", "#252525", gsub("C6RGTANXX", "#D9D9D9", desMat$Tech2)))
...
ggplot(datMelt, aes(x = reorder(ID_Rob, as.numeric(Tech1)), y = value)) +
	theme_minimal() +
	geom_bar(stat = "identity", aes(fill = as.factor(Tech1))) +
	facet_wrap( ~ X2, nrow = 2) +
	theme(axis.text.x = element_text(angle = 90, vjust = 0)) + 
...
ggplot(datMelt, aes(x = reorder(ID_Rob, as.numeric(Tech2)), y = value)) +
	theme_minimal() +
	geom_bar(stat = "identity", aes(fill = as.factor(Tech2))) +
	facet_wrap( ~ X2, nrow = 2) +
	theme(axis.text.x = element_text(angle = 90, vjust = 0)) + 
#	scale_x_discrete(labels = desMat$ID_Rob, name = "") +
	scale_y_continuous("log10(RPKM)") +
...
*%*%*%* To do: Compare PCAs why one over the other?

## PCA
sum(is.na(Exp)) 
NoNA.scaled <- t(scale(t(log10(Exp+1)), scale=F, center=T))

PCA<-prcomp(NoNA.scaled, center=F, scale=F)
Loadings<-as.data.frame(unclass(PCA$rotation))
vars<-PCA$sdev^2
Importance<-vars/sum(vars)

pcaDes <- cbind(desMat, PCA$rotation[rownames(desMat), 1:14])
head(pcaDes)
plot1 <- ggplot(pcaDes, aes(x = PC1, y = PC2, color = Sex, label = ID_Rob)) +
	geom_text(size = 4, hjust=0, vjust=0) +
	geom_point(size = 4, aes(shape = as.factor(trimester)))
plot2 <- ggplot(pcaDes, aes(x = PC3, y = PC4, color = Sex, label = ID_Rob)) +
	geom_text(size = 4, hjust=0, vjust=0) +
	geom_point(size = 4, aes(shape = as.factor(trimester)))
plot3 <- ggplot(pcaDes, aes(x = PC5, y = PC8, color = Sex, label = ID_Rob)) +
	geom_text(size = 4, hjust=0, vjust=0) +
	geom_point(size = 4, aes(shape = as.factor(trimester)))
plot4 <- ggplot(pcaDes, aes(x = PC9, y = PC10, color = Sex, label = ID_Rob)) +
	geom_text(size = 4, hjust=0, vjust=0) +
	geom_point(size = 4, aes(shape = as.factor(trimester)))
plot5 <- ggplot(pcaDes, aes(x = PC11, y = PC12, color = Sex, label = ID_Rob)) +
	geom_text(size = 4, hjust=0, vjust=0) +
	geom_point(size = 4, aes(shape = as.factor(trimester)))
plot6 <- ggplot(pcaDes, aes(x = PC13, y = PC14, color = Sex, label = ID_Rob)) +
	geom_text(size = 4, hjust=0, vjust=0) +
	geom_point(size = 4, aes(shape = as.factor(trimester)))
grid.arrange(plot1, plot2, plot3, plot4, plot5, plot6, ncol=3)

plot1 <- ggplot(pcaDes, aes(x = PC1, y = PC2, color = ProcessingTime, label = ID_Rob)) +
	geom_text(size = 4, hjust=0, vjust=0) +
	geom_point(size = 4, aes(shape = Tech1))
plot2 <- ggplot(pcaDes, aes(x = PC3, y = PC4, color = ProcessingTime, label = ID_Rob)) +
	geom_text(size = 4, hjust=0, vjust=0) +
	geom_point(size = 4, aes(shape = Tech1))
plot3 <- ggplot(pcaDes, aes(x = PC5, y = PC8, color = ProcessingTime, label = ID_Rob)) +
	geom_text(size = 4, hjust=0, vjust=0) +
	geom_point(size = 4, aes(shape = Tech1))
plot4 <- ggplot(pcaDes, aes(x = PC9, y = PC10, color = ProcessingTime, label = ID_Rob)) +
	geom_text(size = 4, hjust=0, vjust=0) +
	geom_point(size = 4, aes(shape = Tech1))
plot5 <- ggplot(pcaDes, aes(x = PC11, y = PC12, color = ProcessingTime, label = ID_Rob)) +
	geom_text(size = 4, hjust=0, vjust=0) +
	geom_point(size = 4, aes(shape = Tech1))
plot6 <- ggplot(pcaDes, aes(x = PC13, y = PC14, color = ProcessingTime, label = ID_Rob)) +
	geom_text(size = 4, hjust=0, vjust=0) +
	geom_point(size = 4, aes(shape = Tech1))
grid.arrange(plot1, plot2, plot3, plot4, plot5, plot6, ncol=3)


source("C:/450k analyses/MTHFR project/heat.scree.R")
meta <- merge(pcaDes, t(col.statsLog), by = 0)
meta_categorical<-meta[,c(2,3,11)]  # input column numbers in meta that contain categorical variables
meta_continuous<-meta[,c(9,10,12,28,29,30,31,32,33)] # input column numbers in meta that contain continuous variables
colnames(meta_categorical)<-c( "Tech1","Tech2","Sex")
colnames(meta_continuous)<-c("GA","trimester", "Processing Time", "count", "mean" ,          "median"  ,      
 "max"     ,       "sd" ,            "mad")

#Apply function on PCA results, pulls in the meta data and beta values from above
heat_scree_plot(Loadings, Importance)
	# PC1 associated with ProcessingTim
	# PC2 associated with Tech1

#t(t(pca$x %*% t(pca$rotation)) + pca$center)
PC1.recon <- PCA$x[,1] %*% t(PCA$rotation[,1]) + PCA$center
PC2.recon <- PCA$x[,2] %*% t(PCA$rotation[,2]) + PCA$center


trunc <- PCA$x[,1:2] %*% t(PCA$rotation[,1:2])
...
meta_categorical<-desMat[,c(1,2,10,13)]  # input column numbers in meta that contain categorical variables
meta_continuous<-desMat[,c(8,9,11)] # input column numbers in meta that contain continuous variables
colnames(meta_categorical)<-c( "Tech1","Tech2","Sex", "Trimester (factor)")
...
```
# Cell means paramaterization
(mod.3<- model.matrix(~ 0 + trimester + Sex + ProcessingTime, modMat))
dmFit.3 <- lmFit(log10(Exp + 1), mod.3)
(contrast.matrix.3 <- makeContrasts(sex = SexMALE, 
																		T1_T2 = trimester1 - trimester2,
																		T1_T3 = trimester1 - trimester3,
																		T3_T2 = trimester3 - trimester2,
                               			levels=colnames(mod.3)))
fitContrasts.3 <- contrasts.fit(dmFit.3, contrast.matrix.3)
ebFit.3<-eBayes(fitContrasts.3)

		# piRNAs where sex is significant
		sex.3 <- topTable(ebFit.3, coef = "sex", adjust.method="BH", number=Inf)
		head(sex.3)
		hist(sex.3$P.Value, breaks = 100)

		# piRNAs where something changes betweeb trimesters 
		All.T.3 <- topTable(ebFit.3, coef = grep("_T", colnames(coef(ebFit.3))), adjust.method="BH", number=Inf)
		head(All.T.3)
		hist(All.T.3$P.Value, breaks = 100)
...
hist(hitsGA$T1.T2, breaks = 30, xlim = c(0,3))
hist(hitsGA$T3.T2, breaks = 30, xlim = c(0,3))
hist(hitsGA$T1.T3, breaks = 30, xlim = c(0,3))
```
...
#Reference paramaterization
(mod<- model.matrix(~ trimester + Sex + Comp.1 + Comp.2, modMat))
dmFit <- lmFit(log10(Exp + 1), mod)
colnames(coef(dmFit))
ebFit<-eBayes(dmFit)

		# piRNAs where sex is significant 
		sex <- topTable(ebFit, coef = "SexMALE", adjust.method="BH", number=Inf)
		head(sex)
		hist(sex$P.Value, breaks = 100)

		# piRNAs where trimester is significant 
		GA <- topTable(ebFit, coef = grep("trimester", colnames(coef(ebFit))), adjust.method="BH", number=Inf)
		head(GA)
		hist(GA$P.Value, breaks = 100)
		
		# piRNAs where T2 is significant (STAT540 - Lecture 10, pg 55)
		T1_T2 <- topTable(ebFit, coef = "trimester2", adjust.method="BH", number=Inf)
		head(T1_T2)
...
# Cell means paramaterization - Sex only
(mod.3<- model.matrix(~ 0 + Sex + Comp.1 + Comp.2, modMat))
dmFit.3 <- lmFit(log10(Exp + 1), mod.3)
(contrast.matrix.3 <- makeContrasts(sex = SexMALE - SexFEMALE, 
																		levels=colnames(mod.3)))
fitContrasts.3 <- contrasts.fit(dmFit.3, contrast.matrix.3)
ebFit.3<-eBayes(fitContrasts.3)

		# piRNAs where sex is significant - this is NOT THE SAME as reference paramterization
		sex.3 <- topTable(ebFit.3, coef = "sex", adjust.method="BH", number=Inf)
		head(sex.3)
		hist(sex.3$P.Value, breaks = 100)
...
# 2. R-Squared
sst <- rowSums(log10(Exp + 1)^2)
ssr <- sst-dmFit$df.residual * dmFit$sigma^2
mod.rs <- (ssr/sst)
hist(mod.rs)
...
(mod <- model.matrix(~ as.factor(trimester) + Sex + Tech1, desMat))
...
(mod_sex <- model.matrix(~ Sex + as.factor(trimester) + Tech1, desMat))
